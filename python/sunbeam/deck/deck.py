...
class Deck(MutableSequence, Hashable):
    ...
    def __delitem__(self, key):
        if isinstance(key, int) or isinstance(key, slice):
            ...
            raise TypeError("Deck indices must be integers or slices, not " +
                            type(key).__name__)
    ...
    def data_invariant(self):
        for kw in self._kws:
            assert isinstance(
                kw, tuple
            ), "Keywords must be tuple instances. Got " + str(type(kw))
            assert len(kw) > 0, "Empty keyword in deck"
            assert isinstance(
                kw[0], str
            ), "The first element of a keyword must be the keyword name of type str. Got type " + str(
                type(kw[0]))
            for record in kw[1:]:
                assert isinstance(
                    record,
                    list), "All keyword records must be lists. Got " + str(
                        type(record))
                for item in record:
                    assert isinstance(
                        item, (int, float, str, _repeat)
                    ), " Keyword items must be either int, float, str, default or repeat. Got " + str(
                        type(item))
